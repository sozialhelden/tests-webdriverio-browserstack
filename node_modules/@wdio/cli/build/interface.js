...
class WDIOCLInterface extends _events.default {
  ...
  printSummary() {
    const totalJobs = this.totalWorkerCnt;
    return this.interface.log('Test Suites:\t', _chalk.default.green(this.result.passed, 'passed') + ', ' + (this.result.failed ? _chalk.default.red(this.result.failed, 'failed') + ', ' : '') + totalJobs, 'total', `(${totalJobs ? Math.round(this.result.finished / totalJobs * 100) : 0}% completed)`);
  }
